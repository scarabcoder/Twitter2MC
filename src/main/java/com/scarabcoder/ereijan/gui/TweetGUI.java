	...
	public void sendTweet(){
		try{
        	...
        	if(this.image){
        		...
        		Events.tick = mc.theWorld.getTotalWorldTime();
        		Events.text = this.text.getText();
        		Events.check = true;
    		}else{

        		ScarabUtil.chat("Posted following tweet:");
    			StatusUpdate update = new StatusUpdate(this.text.getText());
        		Main.twitter.updateStatus(update);
        		ScarabUtil.chat("@" + Main.twitter.getScreenName() + ": " + this.text.getText());
        		mc.thePlayer.closeScreen();
    		}
    		
    		
    	}catch(Exception e){
    		ScarabUtil.chat("Error sending tweet! Check logs for more information.");
    		e.printStackTrace();
    	}
	}
